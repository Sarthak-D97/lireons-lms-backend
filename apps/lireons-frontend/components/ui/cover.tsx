...
export const Cover = ({
    ...
    const [beamConfig, setBeamConfig] = useState<Array<{ duration: number; delay: number }>>([]);
    const ref = useCallback((node: HTMLDivElement | null) => {
        if (node !== null) {
            ...
            const configs = positions.map(() => ({
                duration: Math.random() * 2 + 1,
                delay: Math.random() * 2 + 1,
            }));
            setBeamConfig(configs);
        }
    }, []);
    ...
    return (
        <div
            ...
            ref={ref}
            // CHANGED HERE: 
            // - bg-transparent: Keeps it clear normally.
            // - hover:bg-neutral-900: Turns very dark (almost black) on hover.
        ...
        >
            ...
            {beamPositions.map((position, index) => (
                <Beam
                    key={index}
                    hovered={hovered}
                    duration={beamConfig[index]?.duration ?? 2}
                    delay={beamConfig[index]?.delay ?? 1}
